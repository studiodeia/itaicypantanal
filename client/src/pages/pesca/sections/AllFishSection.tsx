...
const FISH_PER_PAGE = 9;

interface AllFishSectionProps {
  allFish: FishSpecies[];
  categories: string[];
}

export const AllFishSection = ({
  allFish,
  categories,
}: AllFishSectionProps): JSX.Element => {
  ...
  const filteredFish = useMemo(() => {
    ...
    if (activeCategory !== "Todas") {
      fish = fish.filter((f) => f.category === activeCategory);
    }
  ...
  const totalPages = Math.max(1, Math.ceil(filteredFish.length / FISH_PER_PAGE));
  const paginatedFish = filteredFish.slice(
    (currentPage - 1) * FISH_PER_PAGE,
    currentPage * FISH_PER_PAGE,
  );
          ...
          {paginatedFish.length > 0 ? (
            ...
            >
              {paginatedFish.map((fish) => (
                <motion.div key={fish.slug} variants={cardItem}>
